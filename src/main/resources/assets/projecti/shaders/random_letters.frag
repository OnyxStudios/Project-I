...
uniform int ticks;
vec3 color = vec3(0.2, 0.42, 0.67); // blue 1
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
    ...
    vec2 uv = p / resolution.xy - 0.5;
    p.y += 120.0*ticks;
    float r = rand(floor(p/8.0));
    vec2 ip = mod(p,8.0)-4.0;

    float a = -0.3*smoothstep(0.1, 0.8, length(uv)) +
    invader(ip, 809999.0*r) * (0.06 + 0.3*ring(uv,r) + max(0.0, 0.2*sin(10.0*r*ticks)));
